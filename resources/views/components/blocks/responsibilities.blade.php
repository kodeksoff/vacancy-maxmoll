<section class="responsibilities" id="responsibilities">
    <div class="container xl:max-w-7xl">
        <div class="section-header">
            <div class="section-header__title font-custom">Обязанности и требования</div>
        </div>
        <div class="section-content">
            <div class="flex flex-col lg:flex-row">
                <div class="section-content__left flex flex-col items-center lg:items-start">
                    <div class="mt-7">
                        <div class="section-content__subtitle">Чем предстоит заниматься:</div>
                    </div>
                </div>
                <div class="section-content__right">
                    <div class="responsibilities-item flex flex-row">
                        <div class="responsibilities-item__integer font-custom">01</div>
                        <div class="responsibilities-item__description">
                            Осуществление приема на склад, хранения, отправок и выдачи со склада материальных ценностей;
                        </div>
                    </div>
                    <div class="responsibilities-item flex flex-row">
                        <div class="responsibilities-item__integer font-custom">02</div>
                        <div class="responsibilities-item__description">
                            Осуществление приема на склад, хранения, отправок и выдачи со склада материальных ценностей;
                        </div>
                    </div>
                    <div class="responsibilities-item flex flex-row">
                        <div class="responsibilities-item__integer font-custom">03</div>
                        <div class="responsibilities-item__description">
                            Комплектация товаров;
                        </div>
                    </div>
                    <div class="responsibilities-item flex flex-row">
                        <div class="responsibilities-item__integer font-custom">04</div>
                        <div class="responsibilities-item__description">
                            Осуществление приема на склад, хранения, отправок и выдачи со склада материальных ценностей;
                        </div>
                    </div>
                    <div class="responsibilities-item flex flex-row">
                        <div class="responsibilities-item__integer font-custom">05</div>
                        <div class="responsibilities-item__description">
                            Осуществление приема на склад, хранения, отправок и выдачи со склада материальных ценностей;
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex flex-col lg:flex-row mt-20">
                <div class="section-content__left flex flex-col items-center lg:items-start">
                    <div class="mt-7">
                        <div class="section-content__subtitle">Мы не требуем опыт работы кладовщиком, но мы ждем, что ты будешь:</div>
                    </div>
                </div>
                <div class="section-content__right">
                    <div class="responsibilities-item flex flex-row">
                        <div class="responsibilities-item__integer font-custom">01</div>
                        <div class="responsibilities-item__description">
                            Осуществление приема на склад, хранения, отправок и выдачи со склада материальных ценностей;
                        </div>
                    </div>
                    <div class="responsibilities-item flex flex-row">
                        <div class="responsibilities-item__integer font-custom">02</div>
                        <div class="responsibilities-item__description">
                            Осуществление приема на склад, хранения, отправок и выдачи со склада материальных ценностей;
                        </div>
                    </div>
                    <div class="responsibilities-item flex flex-row">
                        <div class="responsibilities-item__integer font-custom">03</div>
                        <div class="responsibilities-item__description">
                            Осуществление приема на склад, хранения, отправок и выдачи со склада материальных ценностей;
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
